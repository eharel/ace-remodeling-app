  ...
  try {
    ...
    for (const file of files) {
      ...
      if (data.projects && Array.isArray(data.projects)) {
        ...
        console.log(`✅ Loaded ${data.projects.length} projects from ${file}`);
      }
    }

    console.log(
      `✅ Total: ${uploadedProjects.length} uploaded projects from Firebase Storage`
    );
...
async function addSeedProjects(): Promise<{
  ...
  for (const project of projectsToSeed) {
    try {
      ...
      console.log(
        `   🎨 Added: "${project.name}" (${project.category}${subcategoryString}, ${photoCount}${docString}) - ID: ${docRef.id}`
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
